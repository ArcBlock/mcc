defmodule Mcc.Expiration.Worker do
  ...
  def handle_info(
    ...
    clean_table_via_size_limit(exp_tab, main_tab, size_limit)
    clean_table_via_memory_limit(exp_tab, main_tab, memory_limit)
    scheduler()
    {:noreply, state}
  end
  ...
  @doc false
  defp clean_table_via_size_limit(exp_tab, main_tab, size_limit) do
    if main_tab.table_info(:size) >= size_limit do
      size_waterline = round(size_limit * 0.7)
      clean_table_via_limit(exp_tab.first(), :size, exp_tab, main_tab, size_waterline)
    end
  end

  @doc false
  defp clean_table_via_memory_limit(exp_tab, main_tab, memory_limit) do
    memory_limit = round(memory_limit * 1024 * 1024 / 8)
    mem_waterline = round(memory_limit * 0.7)
  ...
  @doc false
  defp scheduler, do: Process.send_after(self(), :tick, 1_000)
